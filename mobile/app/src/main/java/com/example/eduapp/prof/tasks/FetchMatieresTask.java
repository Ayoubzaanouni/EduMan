...
public class FetchMatieresTask extends AsyncTask<Void, Void, Map<Integer, String>> {

    private FetchMatieresTaskListener listener;

    public FetchMatieresTask(FetchMatieresTaskListener listener) {
    ...
    protected Map<Integer, String> doInBackground(Void... params) {
        ...
        try {
            URL url = new URL(IP_ADRESS+"/api/profs/matieres"); // Update with your actual server address
    ...
    protected void onPostExecute(Map<Integer, String> textValues) {
        ...
        if (listener != null) {
            listener.onFetchMatieresCompleted(textValues);
        }
    }

    // Interface to notify the calling class about the completion
    public interface FetchMatieresTaskListener {
        void onFetchMatieresCompleted(Map<Integer, String>textValues);
    }
}
